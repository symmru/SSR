...
def export_partial_spheres(int_list, dest_folder):
    if not os.path.isdir(dest_folder):
        os.makedirs(dest_folder)
    fname = os.path.join(dest_folder, "partial_icosphere_{}.pkl")
    for i in int_list:
        s = partial_icosphere(i)
        s.export_mesh_info(fname.format(i))
...
class icosphere(object):
    def __init__(self, level=0, upward=False):
        ...
        self.info = {"V": self.vertices,
                     ...
                     "Seq": self.Seq,
                     "Intp": self.Intp,
                     # maps new faces from level L to level L+1 to the vertices used to derive the new mid-point vertices 
                     "separated_src_idx": self.separated_src_idx,
                     # maps edges to unique edges associated with vertices in level L+1
                     "unique": self.unique}
    ...
    def subdivide(self):
        ...
        self.separated_src_idx = [faces[:, g] for g in [[0, 1], [1, 2], [2, 0]]]
        
        src_idx = np.vstack(self.separated_src_idx)
    ...
    def construct_matrices(self):
        ...
        G = igl.grad(V, F)
        L = igl.cotmatrix(V, F)
        N = igl.per_face_normals(V, F, np.ones(3)/np.linalg.norm(np.ones(3)))
...
class partial_icosphere(icosphere):
  def __init__(self, partial_level):
    super().__init__(level=1)

    print(self.faces.shape)

    self.partial_level = partial_level
    for l in range(2, self.partial_level+1):
      self.subdivide_face()
      self.normalize()
    ...
    EE = 3
    FF = 1
    for i in range(2, self.partial_level):
      VV, EE, FF = updateVEF(VV, EE, FF)
  ...
  def subdivide_face(self):
    """
    Subdivide a mesh into smaller triangles.
    """
    faces = self.faces
    vertices = self.vertices
    face_index = np.arange(79, len(faces))
    ...
    f = np.column_stack([faces[:, 0], mid_idx[:, 0], mid_idx[:, 2],
                         ...
                         mid_idx[:, 0], mid_idx[:, 1], mid_idx[:, 2], ]).reshape((-1, 3))
    # add the 3 new faces per old face
    new_faces = np.vstack((self.faces[:79], faces, f[len(face_index):]))
    # replace the old face with a smaller face
    new_faces[face_index] = f[:len(face_index)]
...
if __name__ == "__main__":
  with open('mesh_files/icosphere_1.pkl', 'rb') as fb:
    old = pickle.load(fb)

  with open('mesh_files/new_icosphere_1.pkl', 'rb') as fb:
    new = pickle.load(fb)
  ...
  for k, v in old.items():
    if k not in new:
      ...
      continue
    if type(v) == type((0,)):
      for vo, vn in zip(v, new[k]):
        assert np.all(np.abs(vo - vn) < 1e-6)
      continue
    try:
      vo = v.todense()
      vn = new[k].todense()
    ...
    try:
      ...
      print( np.all(np.abs(vo - vn) < 1e-6))
